...
const featureStyles = [
  { icon: Camera, color: "text-sky-500", bg: "bg-sky-500/10" },
  { icon: Timer, color: "text-rose-500", bg: "bg-rose-500/10" },
  { icon: Layers, color: "text-emerald-500", bg: "bg-emerald-500/10" },
  { icon: GraduationCap, color: "text-amber-500", bg: "bg-amber-500/10" },
  { icon: Calculator, color: "text-primary", bg: "bg-primary/10" },
  { icon: ScanLine, color: "text-cyan-500", bg: "bg-cyan-500/10" },
  { icon: BookOpen, color: "text-fuchsia-500", bg: "bg-fuchsia-500/10" },
        ...
        <section className="mx-auto grid w-full max-w-6xl items-center gap-12 py-12 lg:grid-cols-[1.1fr_1fr] lg:py-20">
          <div className="space-y-6">
            ...
            <div className="animate-fade-up anim-d3 flex flex-wrap gap-3 pt-2">
              ...
              >
                {t.landing.freeStart} <ArrowRight size={16} />
              </Link>
              <Link
                href="/login"
            ...
            <div className="animate-fade-up anim-d4 flex flex-wrap items-center gap-x-5 gap-y-2 pt-2 text-xs text-muted-foreground">
              {t.landing.proofs.map((proof) => (
                <span key={proof} className="flex items-center gap-1.5">
                  <CheckCircle2 size={13} className="text-emerald-500" /> {proof}
                </span>
              ))}
            </div>
          </div>
          ...
          <div className="mt-8 grid gap-4 lg:grid-cols-3">
            <div className="bg-summit group relative flex flex-col justify-between overflow-hidden rounded-3xl border border-border p-7 shadow-soft lg:col-span-1 lg:row-span-2">
              ...
              <div className="relative">
                ...
                <h3 className="mt-4 font-display text-xl font-bold">{t.spotlight.title}</h3>
                <p className="mt-2 text-sm text-muted-foreground">{t.spotlight.body}</p>
              </div>
            ...
            <div className="grid gap-4 sm:grid-cols-2 lg:col-span-2 lg:grid-cols-4">
              {featureStyles.map(({ icon: Icon, color, bg }, index) => (
                <div
                ...
                >
                  ...
                  <h3 className="mt-3 font-semibold">{t.features[index][0]}</h3>
                  <p className="mt-1 text-sm text-muted-foreground">{t.features[index][1]}</p>
                </div>
              ))}
            </div>
          </div>
...
function AppPreview({
  ...
  copy: ReturnType<typeof getDict>["landing"];
  daysToYks: number;
}) {
  return (
    <div className="w-full max-w-md rounded-3xl border border-border bg-card p-4 shadow-pop">
      <div className="flex items-center justify-between rounded-2xl border border-orange-500/25 bg-gradient-to-r from-orange-500/12 to-transparent p-3.5">
        ...
        <Flame size={28} className="animate-ember text-orange-500" />
      </div>
      ...
      <div className="mt-3 rounded-2xl border border-border bg-background p-3.5">
        <div className="mb-2 flex items-center justify-between text-xs">
          <span className="font-semibold">{copy.tasksTitle}</span>
      ...
      <div className="mt-3 flex items-center gap-2 rounded-2xl bg-primary/10 p-3 text-xs text-primary">
        <Sparkles size={14} /> {copy.aiPrompt}
      </div>
    </div>
  );
}
