...
import { StreakNudgeClient } from "./streak-nudge-client";
...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("total_xp, avatar_url, display_name")
    .eq("id", user.id)
    .single();
  ...
  return (
    <div className="flex min-h-screen">
      <Sidebar logoutAction={logoutAction} labels={t} userId={user.id} locale={locale} />
      <div className="flex flex-1 flex-col min-w-0">
        <header className="sticky top-0 z-20 flex h-14 items-center justify-between border-b border-border bg-background/70 px-4 pl-14 backdrop-blur-xl lg:pl-6">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <StreakDisplay userId={user.id} locale={locale} />
            ...
            >
              {profile?.avatar_url ? (
                <img src={profile.avatar_url} alt={t.profile} className="h-full w-full object-cover" />
              ) : (
      ...
      <StreakNudgeClient userId={user.id} labels={dict.streakNudge} />
    </div>
  );
}